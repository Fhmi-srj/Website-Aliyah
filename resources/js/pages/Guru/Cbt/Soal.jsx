...
export default function GuruCbtSoal() {
    ...
    const fetchBankAndQuestions = async () => {
        try {
            ...
            const [bankRes, questionsRes] = await Promise.all([
                authFetch(`${API_BASE}/guru-panel/cbt/question-banks/${id}`),
                authFetch(`${API_BASE}/guru-panel/cbt/question-banks/${id}/questions`)
            ]);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const url = modalMode === 'add' ? `${API_BASE}/guru-panel/cbt/question-banks/${id}/questions` : `${API_BASE}/guru-panel/cbt/questions/${currentItem.id}`;
    ...
    const handleDelete = async (qId) => {
        ...
        try {
            const response = await authFetch(`${API_BASE}/guru-panel/cbt/questions/${qId}`, { method: 'DELETE' });
            if (response.ok) {
                fetchBankAndQuestions();
    ...
    const handleImportUpload = async () => {
        ...
        try {
            ...
            const response = await authFetch(`${API_BASE}/guru-panel/cbt/question-banks/${id}/questions/bulk`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
        ...
        <div className="animate-fadeIn flex flex-col flex-grow max-w-full overflow-auto">
            <header className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    <button onClick={() => navigate('/guru/cbt/bank-soal')} className="w-10 h-10 rounded-xl flex-shrink-0 bg-white border border-gray-200 text-gray-500 hover:text-green-600 hover:border-green-600 flex items-center justify-center transition-all shadow-sm">
                    ...
                    return (
                        <div key={q.id} className="bg-white p-6 rounded-2xl shadow-soft border border-gray-100 relative group">
                            ...
                            <div className="flex gap-4">
                                <div className="w-8 h-8 rounded-full bg-green-500/10 text-green-600 flex items-center justify-center font-black flex-shrink-0">
                                    {idx + 1}
                                </div>
