...
export default function GuruHasilUjian() {
    ...
    const fetchResults = async () => {
        try {
            setLoading(true);
            const res = await authFetch(`${GURU_BASE}/cbt/exams/${id}/results`);
            if (res.ok) {
    ...
    const openGradeModal = async (session) => {
        ...
        try {
            ...
            const res = await authFetch(`${GURU_BASE}/cbt/exams/${id}/results/${session.siswa_id}`);
            if (res.ok) {
                const data = await res.json();
    ...
    const handleSaveGrades = async () => {
        ...
        try {
            ...
            const res = await authFetch(`${GURU_BASE}/cbt/exams/${id}/results/${selectedSession.siswa_id}/grade`, {
                method: 'POST',
                                                        ...
                                                        <div className="w-full md:w-36 flex-shrink-0">
                                                            <div className={`rounded-xl border p-4 text-center ${isEssay ? 'border-amber-200 bg-amber-50' : 'border-blue-100 bg-blue-50/50'}`}>
                                                                ...
                                                                <input 
                                                                    ...
                                                                    className={`w-full text-center font-black text-xl p-2 border rounded-lg outline-none transition-all ${isEssay ? 'border-amber-300 focus:ring-2 focus:ring-amber-400 bg-white' : 'border-gray-300 bg-gray-100 cursor-not-allowed'}`}
                                                                    disabled={!isEssay}
                                                                />
