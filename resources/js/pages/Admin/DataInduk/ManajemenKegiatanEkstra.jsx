...
function ManajemenKegiatanEkstra() {
    ...
    const periodeKegiatanList = ['Harian', 'Mingguan', 'Bulanan'];
    ...
    const fetchData = async (tahunId = tahunAjaranId) => {
        ...
        try {
            ...
            const [kegiatanRes, guruRes, kelasRes, kalenderRes] = await Promise.all([
                authFetch(`${API_BASE}/kegiatan-rutin${taParam}`),
    ...
    const openEditModal = (item) => {
        ...
        setFormData({
            ...
            jenis_kegiatan: item.jenis_kegiatan || 'Mingguan',
    ...
    const handleBulkDelete = async (force = false) => {
        ...
        try {
            const response = await authFetch(`${API_BASE}/kegiatan-rutin/bulk-delete`, {
                method: 'POST',
    ...
    const handleExport = () => {
        const exportData = filteredData.map((item, idx) => ({
            ...
            'Periode': item.jenis_kegiatan,
            'Mulai': item.hari + " " + (item.jam_mulai?.substring(0,5)||""),
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const url = modalMode === 'add' ? `${API_BASE}/kegiatan-rutin` : `${API_BASE}/kegiatan-rutin/${currentItem.id}`;
    ...
    const handleDelete = async (id, force = false) => {
        ...
        try {
            const url = force ? `${API_BASE}/kegiatan-rutin/${id}?force=true` : `${API_BASE}/kegiatan-rutin/${id}`;
    ...
    const openAbsensiModal = async (item) => {
        ...
        try {
            ...
            const res = await authFetch(`${API_BASE}/kegiatan-rutin/${item.id}/absensi-admin`);
    ...
    return (
        <div className="animate-fadeIn flex flex-col flex-grow max-w-full overflow-auto">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="page-header-subtitle text-xs text-gray-400 mt-0.5 font-medium uppercase tracking-widest">Atur kegiatan ekstra & event spesial sekolah</p>
                        </div>
                    </div>
                </div>
            </header>
                    ...
                    <table className={`admin-table ${isMobile ? '' : 'min-w-[1200px]'}`}>
                        <thead>
                            <tr>
                                ...
                                {!isMobile && (
                                    <>
                                        <SortableHeader label="Hari" column="hari" />
                                        <SortableHeader label="Mulai" column="jam_mulai" />
                                    </>
                                )}
                                {!isMobile && <SortableHeader label="Selesai" column="jam_selesai" />}
                                        ...
                                        {!isMobile && (
                                            <td className="py-2.5 px-2 align-middle whitespace-nowrap">
                                                <div className="flex items-center gap-1.5 text-xs font-bold text-gray-600 dark:text-gray-400">
                                                    ...
                                                    <span className="uppercase tracking-tight">{item.hari + " " + (item.jam_mulai?.substring(0,5)||"")}</span>
                                                </div>
                                            </td>
                                        )}
                                        {!isMobile && (
                                            <td className="py-2.5 px-2 align-middle whitespace-nowrap">
                                                <div className="flex items-center gap-1.5 text-xs font-bold text-gray-600 dark:text-gray-400">
                                                    ...
                                                    <span className="uppercase tracking-tight">{item.jam_selesai ? item.jam_selesai.substring(0,5) : '-'}</span>
                                                </div>
                                            </td>
                                        )}
                                        {!isMobile && (
                                        ...
                                        <tr className="bg-gray-50/50 dark:bg-dark-bg/30 border-b border-gray-100 dark:border-dark-border animate-slideDown">
                                            <td colSpan="4" className="p-0">
                                                <div className="mobile-expand-grid">
                                                    <div className="expand-item"><span className="expand-label">Mulai</span><span className="expand-value">{item.hari + " " + (item.jam_mulai?.substring(0,5)||"")}</span></div>
                                                    <div className="expand-item"><span className="expand-label">Selesai</span><span className="expand-value">{item.jam_selesai ? item.jam_selesai.substring(0,5) : '-'}</span></div>
            ...
            >
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div className="space-y-1.5">
                            <label className="block text-[11px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-wide">Periode Kegiatan</label>
                            <select value={formData.jenis_kegiatan} onChange={(e) => setFormData({ ...formData, jenis_kegiatan: e.target.value })} className="input-standard outline-none">
                                {periodeKegiatanList.map(j => <option key={j} value={j}>{j}</option>)}
                            </select>
                        </div>
                ...
                <div>
                    <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-4 px-1">Waktu Kegiatan</label>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                        {formData.jenis_kegiatan !== 'Harian' && (
                            <div className="space-y-1.5">
                                <label className="block text-[11px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-wide">Hari *</label>
                                <select required value={formData.hari} onChange={(e) => setFormData({ ...formData, hari: e.target.value })} className="input-standard outline-none">
                                    {['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu'].map(h => <option key={h} value={h}>{h}</option>)}
                                </select>
                            </div>
                        )}
                        <div className="space-y-1.5">
                            <label className="block text-[11px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-wide">Jam Mulai *</label>
                            <input required type="time" value={formData.jam_mulai} onChange={(e) => setFormData({ ...formData, jam_mulai: e.target.value })} className="input-standard" />
                        </div>
                        <div className="space-y-1.5">
                            <label className="block text-[11px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-wide">Jam Selesai</label>
                            <input type="time" value={formData.jam_selesai} onChange={(e) => setFormData({ ...formData, jam_selesai: e.target.value })} className="input-standard" />
                        </div>
                    </div>
                </div>
                <div>
            ...
            ` }} />
            {/* AbsensiModal is removed since KegiatanRutin is a template */}
        </div>
    );
}
...
function AbsensiKegiatanAdminModal({ show, onClose, kegiatan, initialData, onSuccess, guruList, siswaList = [] }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoading(true);
            const res = await authFetch(`${API_BASE}/kegiatan-rutin/${kegiatan.id}/absensi-admin`, {
                method: 'PUT',
...
export default ManajemenKegiatanEkstra;
