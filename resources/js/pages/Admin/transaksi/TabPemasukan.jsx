...
export default function TabPemasukan({ isMobile }) {
    ...
    const [form, setForm] = useState({ sumber_id: '', nominal: '', keterangan: '', tanggal: new Date().toISOString().split('T')[0], foto: null });
    ...
    const openAdd = () => { setMode('add'); setForm({ sumber_id: '', nominal: '', keterangan: '', tanggal: new Date().toISOString().split('T')[0], foto: null }); setFotoPreview(null); setNominalDisplay(''); setShowModal(true); };
    const openEdit = (item) => { setMode('edit'); setCurrent(item); setForm({ sumber_id: item.sumber_id ? Number(item.sumber_id) : (item.sumber?.id ? Number(item.sumber.id) : ''), nominal: item.nominal, keterangan: item.keterangan || '', tanggal: item.tanggal ? item.tanggal.substring(0, 10) : new Date().toISOString().split('T')[0], foto: null }); setFotoPreview(item.foto ? `/storage/${item.foto}` : null); setNominalDisplay(formatRupiah(item.nominal)); setShowModal(true); };
