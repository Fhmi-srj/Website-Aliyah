...
export default function TabPengeluaran({ isMobile }) {
    ...
    const fetchData = async () => {
        setLoading(true);
        try { const r = await authFetch(`${API_BASE}/pengeluaran`); const d = await r.json(); setList(d.data || []); } catch (e) { console.error(e); } finally { setLoading(false); }
    };

    const fetchKategori = async () => {
        try {
            const [s1, s2] = await Promise.all([
                authFetch(`${API_BASE}/transaksi-kategori?tipe=sumber_pengeluaran`),
                authFetch(`${API_BASE}/transaksi-kategori?tipe=kategori_pengeluaran`),
            ]);
            const d1 = await s1.json(); setSumberList(d1.data || []);
            const d2 = await s2.json(); setKategoriList(d2.data || []);
    ...
    const deleteItem = async (id) => {
        ...
        if (c.isConfirmed) { try { await authFetch(`${API_BASE}/pengeluaran/${id}`, { method: 'DELETE', headers: { Accept: 'application/json' } }); fetchData(); Swal.fire({ icon: 'success', title: 'Terhapus!', timer: 1500, showConfirmButton: false }); } catch { } }
    };
    ...
    const filtered = list.filter(i => {
        if (search) { const s = search.toLowerCase(); if (!(i.keterangan?.toLowerCase().includes(s) || i.sumber?.nama?.toLowerCase().includes(s) || i.kategori?.nama?.toLowerCase().includes(s) || String(i.nominal).includes(s))) return false; }
        if (filterKategori && String(i.kategori?.id) !== String(filterKategori)) return false;
    ...
    const activeFilterCount = (filterKategori ? 1 : 0) + (filterAdmin ? 1 : 0) + (sortBy !== 'terbaru' ? 1 : 0);
    ...
    return (
        <>
                ...
                {isMobile ? (
                    <div className="flex flex-col gap-1.5 w-full">
                        ...
                        {showFilter && (
                            <div className="flex gap-1.5 pt-1">
                                ...
                                <select value={filterKategori} onChange={e => setFilterKategori(e.target.value)} className="flex-1 min-w-0 px-1.5 py-1 rounded-lg text-[8px] font-bold bg-white border border-gray-200 text-gray-600 transition-all">
                    ...
                    <>
                        <div className="flex items-center gap-3 flex-wrap w-full">
                            ...
                            <select value={filterKategori} onChange={e => setFilterKategori(e.target.value)} className="px-3 py-2.5 rounded-xl text-xs font-bold bg-white border border-gray-200 text-gray-600 focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all shadow-sm">
                    ...
                    <div className={`bg-white rounded-2xl shadow-soft border border-gray-100 ${isMobile ? 'overflow-hidden' : 'overflow-x-auto scrollbar-hide'}`}>
                        <table className={`admin-table ${isMobile ? 'mobile-table-fixed' : 'min-w-[800px]'}`}>
                            <thead><tr>
                                ...
                                {!isMobile && <th className="select-none py-2 text-xs font-black text-gray-400 uppercase tracking-widest">Sumber</th>}
                                <th className={`select-none py-2 ${isMobile ? 'px-2 text-[8px]' : 'text-xs'} font-black text-gray-400 uppercase tracking-widest`}>Kategori</th>
                                ...
                                {filtered.map((item, idx) => (
                                    <React.Fragment key={item.id}>
                                        <tr className="hover:bg-gray-50/50 transition-colors border-b border-gray-100 last:border-0 group">
                                            ...
                                            {!isMobile && <td className="py-2 align-middle"><span className="px-2 py-0.5 bg-rose-50 text-rose-700 rounded-lg text-[10px] font-bold">{item.sumber?.nama || '-'}</span></td>}
                                            <td className={`py-2 ${isMobile ? 'px-2' : ''} align-middle`}><span className={`px-2 py-0.5 bg-amber-50 text-amber-700 rounded-lg font-bold ${isMobile ? 'text-[8px]' : 'text-[10px]'}`}>{item.kategori?.nama || '-'}</span></td>
                                            ...
                                            <tr className="bg-gray-50/50 border-b border-gray-100">
                                                <td colSpan="5" className="p-0">
                                                    <div className="mobile-expand-grid">
                                                        <div className="expand-item"><span className="expand-label">Sumber</span><span className="expand-value">{item.sumber?.nama || '-'}</span></div>
                        ...
                        <div className={`flex items-center justify-between border-t border-gray-100 bg-gray-50/30 ${isMobile ? 'p-3' : 'p-6'}`}>
                            <span className={`font-black text-gray-400 uppercase tracking-widest flex items-center gap-1.5 ${isMobile ? 'text-[9px]' : 'text-[11px]'}`}><div className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse"></div>Total: Rp {fmt(filtered.reduce((s, i) => s + parseFloat(i.nominal || 0), 0))}</span>
            ...
            <CrudModal show={showModal} onClose={() => setShowModal(false)} title={mode === 'add' ? 'Tambah Pengeluaran' : 'Edit Pengeluaran'} subtitle="Catat pengeluaran keuangan" icon={mode === 'add' ? 'plus' : 'edit'} onSubmit={submit} submitLabel={mode === 'add' ? 'Simpan' : 'Perbarui'} isMobile={isMobile}>
                <ModalSection label="Detail Pengeluaran" isMobile={isMobile} />
                <div className="grid grid-cols-1 gap-4">
                    <div className={`grid gap-4 ${isMobile ? 'grid-cols-1' : 'grid-cols-2'}`}>
                        <div className="space-y-1">
                            <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Sumber *</label>
                            <SearchableDropdown options={sumberList} value={form.sumber_id} onChange={val => setForm({ ...form, sumber_id: val })} onAddNew={async (nama) => { const k = await createKategori(nama, 'sumber_pengeluaran'); if (k) { setForm(f => ({ ...f, sumber_id: k.id })); return k; } }} onEdit={editKategori} onDelete={deleteKategori} placeholder="Pilih atau ketik sumber baru..." />
                        </div>
                        <div className="space-y-1">
                            <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider">Kategori *</label>
                            <SearchableDropdown options={kategoriList} value={form.kategori_id} onChange={val => setForm({ ...form, kategori_id: val })} onAddNew={async (nama) => { const k = await createKategori(nama, 'kategori_pengeluaran'); if (k) { setForm(f => ({ ...f, kategori_id: k.id })); return k; } }} onEdit={editKategori} onDelete={deleteKategori} placeholder="Pilih atau ketik kategori baru..." />
                        </div>
                    </div>
                    <div className={`grid gap-4 ${isMobile ? 'grid-cols-1' : 'grid-cols-2'}`}>
                        <div className="space-y-1">
