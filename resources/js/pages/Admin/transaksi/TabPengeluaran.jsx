...
export default function TabPengeluaran({ isMobile }) {
    ...
    const filtered = list.filter(i => { if (!search) return true; const s = search.toLowerCase(); return i.keterangan?.toLowerCase().includes(s) || i.sumber?.nama?.toLowerCase().includes(s) || i.kategori?.nama?.toLowerCase().includes(s) || String(i.nominal).includes(s); });
